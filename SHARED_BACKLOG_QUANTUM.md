...
| S14 | **Conformal concavity holds at n = 1 (entanglement entropy): tan(θ/4)·a(θ) is convex in u = tan²(θ/4).** Derived from SSA plus the vacuum's global conformal invariance, by running Lanzetta–Moult–Wang's eye construction (2609.04302) with SSA on nested regions. Their variational step needs only a 2×2 Cauchy–Schwarz, which SSA supplies at n = 1. (Added 2026-09-24; general statement only, per the bridge.) | **derived at physics-level; checked outside cuspis by the bridge (V7, non-blind); not a theorem** | An algebraic proof. |
