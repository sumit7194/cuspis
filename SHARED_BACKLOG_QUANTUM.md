...
*This is the quantum-safe copy of `SHARED_BACKLOG.md`, redacted on 2026-09-24: some rows and passages were removed and are not listed.*
...
| S4 | The only known "second channel" is Lanzetta–Moult–Wang's rectangle of twist lines. It gives κ_n ≥ (2π/3)·a_n(π/2) at integer n ≥ 2. **At n = 1:**<br>• SSA on equal-height rectangles plus 90° rotation gives **κ ≥ 2·a(π/2)**, the n = 1 counterpart of the paper's one-derivative bound. With a(π/2) ≥ (π²C_T/3)·log√2 (Bueno–Witczak-Krempa 2016, SSA plus Lorentz invariance), this gives **κ/C_T ≥ 2.280, unconditionally**. Only 90° data enter.<br>• SSA on rectangles alone cannot give the full 2π/3 at n = 1; that would need Casini–Huerta 2012 conditional positivity for reflected rectangles.<br>• A stronger n = 1 bound follows from S14. It is withheld from this copy pending the sub-45° decision; ask the bridge. | 2.280: derived from standard n = 1 inputs (cuspis EXP-025, 2026-09-24) | — |
...
| S14 | **Conformal concavity holds at n = 1 (entanglement entropy): tan(θ/4)·a(θ) is convex in u = tan²(θ/4).** Derived from SSA plus the vacuum's global conformal invariance, by running Lanzetta–Moult–Wang's eye construction (2609.04302) with SSA on nested regions. Their variational step needs only a 2×2 Cauchy–Schwarz, which SSA supplies at n = 1. (Added 2026-09-24; general statement only, per the bridge.) | **PARTIAL**: derived at physics-level rigour; checked only inside cuspis (a same-session subagent review; an EMI model check). Outside check: bridge V7 | A holographic (RT) check; an algebraic proof. |
...
violates. *Do not contradict:* S1 and S5. Sub-routes already shown to give lower bounds only:
...
**CF-16 Painlevé closed form.** A closed form for the Dirac κ₂ (Bueno–Myers–Witczak-Krempa 2015, App. B). An
integrable-systems curiosity.

**CF-17 Ising Rényi-2 as a multiple of free.** Published ratios to the free scalar: 0.763(22) for κ₂
(1904.08955) and 0.78(4) at 90° (Ngai et al. 2512.00382). *First step:* pre-register the prediction at an angle
nobody has measured, then test it with existing QMC codes.
...
**CF-21.** A parked free-field curiosity.
...
| Upper bound on κ | CF-1 · CF-9 · CF-24 | Each tests a different family of constraints against S5 |
...
**Dependencies.** CF-24 was a cheap first step for CF-1. CF-17's desk study comes before its numerics.
CF-23 should precede any claim about CF-2.

**Suggested fits by repository, for the user and the bridge to decide.**
- **`corner_function`:** the items that use its derivations: CF-9, CF-14, CF-15, CF-24.
...
- **`SpaceTime` and `BlackHole`:** methods review of any new instrument (CF-3, CF-4) against their
