...
> in your pre-registration. (Warning added 2026-09-24.) **Quantum sessions should read `SHARED_BACKLOG_QUANTUM.md`
> instead**, the redacted copy made at the user's request.
...
| S14 | **Conformal concavity holds at n = 1 (entanglement entropy): tan(θ/4)·a(θ) is convex in u = tan²(θ/4).** Derived from SSA plus the vacuum's global conformal invariance, by running Lanzetta–Moult–Wang's eye construction with SSA on nested regions. The key point is that their variational step needs only a 2×2 Cauchy–Schwarz, which SSA supplies at n = 1 for nested regions. It excludes Bueno–Witczak-Krempa's 𝔞_min and this workspace's no-bound witness family (a new n = 1 constraint outside C1–C6). Together with the boosted-wedge inequality (C3), it gives κ/C_T ≥ 2.894 (S4). (Added 2026-09-24.) | **derived at physics-level; checked outside cuspis by the bridge (V7, non-blind); not a theorem.** Assumptions A1–A7 are in report.md EXP-025. Inside cuspis: a same-session subagent review plus an EMI known-answer check to 10⁻¹⁰. Outside: bridge V7 re-derived it (different formalisation, same load-bearing gaps A5, the C¹ junction, and touching as a limit), matched the constants 2.537868 and 2.893629 independently, and its controls fire. Promotion needs proofs of A5 and A6 or a specialist. | A holographic (RT) check of the same identity; an algebraic proof. |
| S15 | **Conformal concavity does not bound κ above either.** An explicit family satisfies C1–C6, CC at n = 1, a₀ = 0, eq. (37) and the θ² sign, with κ/σ up to 3·10⁴. The family is EMI (which carries the mass) + K·(spectral tail from s = w) + one mode at s₀ = 2–3w that cancels the tail's constant at mass cost ∝ e^{−s₀π}. So the no-upper-bound result survives adding S14, with a new witness. A bound needs a constraint that is not pointwise at either end. (Added 2026-09-24, cuspis EXP-024.) | verified numerically at three pre-registered K, with controls that fire; no all-K proof | An all-K proof; any constraint that is not pointwise. |
| S16 | **C11 (conditional positivity for reflected rectangles at n = 1): EMI satisfies it exactly**, so EMI cannot serve as a C11 test. For EMI rectangles g(y) = S′(y) + κ = 2c[(1+1/y²)·arctan(1/y) − 1/y], the Laplace transform of μ(t) = t·Si(t) + sin t/t + cos t − 2 ≥ 0. A real C11 test needs RT rectangles or lattice free fields. (Added 2026-09-24, cuspis EXP-026 §4.) | verified numerically (closed form vs quadrature; Laplace identity to 12 digits; μ ≥ 0 on [10⁻³, 10³] with the right asymptotics) | An RT-rectangle test; this is C11's next step, held as a hedge on V7. |
