...
| S14 | **Conformal concavity holds at n = 1 (entanglement entropy): tan(θ/4)·a(θ) is convex in u = tan²(θ/4).** Derived from SSA plus the vacuum's global conformal invariance, by running Lanzetta–Moult–Wang's eye construction with SSA on nested regions. The key point is that their variational step needs only a 2×2 Cauchy–Schwarz, which SSA supplies at n = 1 for nested regions. It excludes Bueno–Witczak-Krempa's 𝔞_min and this workspace's no-bound witness family (a new n = 1 constraint outside C1–C6). Together with the boosted-wedge inequality (C3), it gives κ/C_T ≥ 2.894 (S4). (Added 2026-09-24.) | **PARTIAL** (bridge index). Derived at physics-level rigour (assumptions A1–A7 in report.md EXP-025). Checked only inside cuspis: an adversarial review by a *subagent of the deriving session*, which shares its framing, and an EMI known-answer check to 10⁻¹⁰ built by that subagent and re-run by the session. Outside check: bridge V7, not blind. | A holographic (RT) check of the same identity; an algebraic proof. |
